<template>
  <div>
    <ul class="product_list">
      <li v-for="product in displayedProducts" :key="product.uid" class="product_item">
...
export default {
  ...
  computed: {
    ...mapState(["products"]),
    pageCount() {
      return Math.ceil(this.products.length / 15);
    },
    displayedProducts() {
...
.product_item {
  ...
  margin-bottom: 20px;
  box-shadow: rgba(100, 100, 111, 0.2) 10px 10px 29px 4px;
  justify-content: space-between;
}
.item_image {
  margin-bottom: 10px;
